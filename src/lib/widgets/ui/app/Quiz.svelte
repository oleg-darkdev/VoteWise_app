...
<div class="max-w-3xl">
	...
	{#each questions as question, i}
		{#if i == quizStep}
			<div class="prose-md prose text-left">
				<div class="flex flex-col ">
					...
					<div class="">
						<button
							...
							class="btn-outline btn-primary btn">{question.options.agree.text}</button
						>
						<button
							on:click={() => {
								quizStep = quizStep + 1;
							}}
							class="btn-outline btn-primary btn">{question.options.disagree.text}</button
						>
					</div>
				</div>
			</div>
